...
const login = async (req, res, next) => {
  ...
  return res
    .status(200)
    .json({ message: "successfully Logged In", name: existingUser.name, token ,email:existingUser.email,
...
const booking= async (req, res) => {
  const { user,carName, from, to, driverRequired, totalDays, totalAmount } = req.body;
...
const getbookingDetail = async (req, res) => {
  try {    
    ...
    // console.log(carName)
    const booked = await Booking.findOne({ cars: carName });    
    if (booked) {
      res.send({ booked, success: true });
    } else {
...
const getOderDetail = async (req, res) => {
  try {    
    ...
    // console.log(uid)
    // console.log(carName)
    const booked = await Booking.find({ user_id: uid });    
    if (booked) {
      res.send({ booked, success: true });
    } else {
      res.send({ success: false });
...
const confirmOrder = async (req, res) => {
  ...
  const { carName, from, to, driverRequired, totalDays, totalAmount,paymethod,uid } = req.body;
      ...
      const oid = order._id;
        ...
        if (paymethod === 'cod') {
         ...
         instance.orders.create(options, (err, orders) => {
          ...
          User.findOne({ user_id: uid }).then((user) => {
...
const verifyPayment = async(req, res) => {
  ...
  if (hmac == details.razorpay_signature) {
   console.log("payment verified!")
    ...
    // console.log(objId);
    Booking.updateOne({ user_id: details.userId }, { $set: { paymentStatus: 'Paid' } })
  .then(() => {
